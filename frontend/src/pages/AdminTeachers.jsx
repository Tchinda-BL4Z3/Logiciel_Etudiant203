...
import { 
  ...
  ClipboardList, Bookmark, BookOpen, Lock
} from 'lucide-react';
...
const AdminTeachers = () => {
  ...
  // État pour la création (Mot de passe ajouté pour la connexion Enseignant)
  const [newTeacher, setNewTeacher] = useState({ 
    ...
    specialite: '',
    ueCode: '', 
    password: '' // Initialisé à vide pour forcer la saisie
  });
  ...
  const [editData, setEditData] = useState({
    ...
    specialite: '',
    ueCode: '',
    password: '' // Permet de réinitialiser le mot de passe
  });
  ...
  const handleEditClick = (teacher) => {
    ...
    setEditData({
      ...
      password: '' // On laisse vide, l'admin ne le remplit que s'il veut le changer
    });
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-black text-[#1E293B] italic uppercase tracking-widest">Gestion des Enseignants</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-50 flex items-center space-x-6">
          <div className="w-16 h-16 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center font-black shadow-inner">
            <UserSquare2 size={32} />
          </div>
          <div>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[2px] mb-1 leading-none">Total Professeurs</p>
            <p className="text-4xl font-black text-slate-800 tracking-tighter">{totalTeachers}</p>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-50 flex items-center space-x-6">
          ...
          <div>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[2px] mb-1 leading-none">Vœux Reçus</p>
            <p className="text-4xl font-black text-cyan-500 tracking-tighter">{pourcentageVoeux}%</p>
          </div>
        </div>
      </div>
                ...
                <React.Fragment key={dept}>
                  ...
                  {groupedTeachers[dept].map((t) => (
                    <tr key={t.id} className="hover:bg-slate-50/30 transition-colors group">
                      ...
                      <td className="px-8 py-6 text-slate-500 font-bold text-xs uppercase tracking-tighter">{t.departement}</td>
                      <td className="px-8 py-6">
      ...
      {/* --- MODALE AJOUT (AVEC CHAMP MOT DE PASSE) --- */}
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[999] p-6 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl rounded-[40px] p-12 shadow-2xl relative border border-white/20">
            ...
            <form onSubmit={handleAddTeacher} className="space-y-6 text-left">
              ...
              {/* Champ Mot de Passe - Crucial pour la connexion Enseignant */}
              <div className="space-y-2">
      ...
      {/* --- MODALE MODIFICATION (AVEC RÉINITIALISATION MOT DE PASSE) --- */}
      {showEditModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[999] p-6 animate-in zoom-in-95 duration-200">
          <div className="bg-white w-full max-w-2xl rounded-[40px] p-12 shadow-2xl relative border-2 border-blue-50">
            ...
            <form onSubmit={handleUpdateTeacher} className="space-y-6 text-left">
              <div className="grid grid-cols-2 gap-6">
                <div>
              ...
              {/* Champ Mot de Passe - Permet de réinitialiser si l'enseignant oublie */}
              <div>
