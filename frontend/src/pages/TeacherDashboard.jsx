...
import { 
  ...
  Bell,
  AlertCircle,
  LogOut // Ajout de l'icône de déconnexion pour le modal
} from 'lucide-react';
...
const TeacherDashboard = () => {
    ...
    const handleFinalLogout = () => {
        localStorage.clear(); // Nettoie tout le stockage
        navigate('/login');
    };
  ...
  return (
    <TeacherLayout onLogout={() => setShowLogoutModal(true)}>
      {/* --- HEADER DYNAMIQUE --- */}
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white rounded-[40px] p-10 max-w-sm w-full shadow-2xl border border-slate-100 text-center animate-in zoom-in-95 duration-300">
                ...
                <h3 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter mb-2">Déconnexion ?</h3>
                ...
                <div className="flex flex-col space-y-3">
                    <button 
                        ...
                        className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-[3px] hover:bg-red-600 transition-all shadow-lg shadow-slate-200"
                    >
                        Oui, me déconnecter
                    </button>
                    <button 
                        onClick={() => setShowLogoutModal(false)}
                        className="w-full bg-slate-50 text-slate-400 py-4 rounded-2xl font-black uppercase text-[10px] tracking-[3px] hover:bg-slate-100 transition-all"
                    >
                        Annuler
                    </button>
                </div>
