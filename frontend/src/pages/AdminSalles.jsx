...
import { 
  ...
  Filter, X, CheckCircle2, AlertTriangle 
} from 'lucide-react';
...
const AdminSalles = () => {
  ...
  const handleUpdateSalle = async (e) => {
    ...
    try {
      ...
      fetchSalles(); // Recharger la liste
  ...
  const filteredSalles = salles.filter(s => 
    ...
    s.batiment.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight italic">Gestion des Salles</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-50">
          ...
          <p className="text-4xl font-black text-[#1d76f2]">
            {filteredSalles.reduce((acc, s) => acc + s.capacite, 0)} <span className="text-sm font-bold text-slate-300 tracking-normal">places</span>
          </p>
        </div>
        <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-50 flex items-center justify-between">
           <div>
            ...
            <p className="text-2xl font-black text-emerald-500">Optimisé</p>
           </div>
      ...
      <div className="bg-white p-4 rounded-[24px] shadow-sm border border-slate-50 mb-8 flex items-center space-x-4">
      ...
      <div className="space-y-12">
        {Object.keys(sallesParDept).length === 0 ? (
          <div className="text-center py-24 bg-white rounded-[32px] border-2 border-dashed border-slate-100 text-slate-400">
            ...
            <p className="font-bold">Aucune salle ne correspond à "{searchQuery}"</p>
          </div>
        ) : (
            ...
            <div key={dept} className="space-y-5">
              <div className="flex items-center space-x-4 ml-2">
                ...
                <h2 className="text-sm font-black text-slate-800 uppercase tracking-widest">
                  Département : <span className="text-blue-600">{dept}</span>
                </h2>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {list.map((salle) => (
                      <tr key={salle.id} className="hover:bg-slate-50/30 transition-colors group">
                        ...
                        <td className="px-8 py-6 text-center">
                          <span className="text-[10px] font-black px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg uppercase border border-emerald-100">
                            Disponible
                          </span>
                        </td>
                        <td className="px-8 py-6 text-right">
                          <div className="flex justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-all">
                            {/* BOUTON MODIFIER BRANCHÉ */}
                            <button 
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[999] p-6">
          <div className="bg-white w-full max-w-xl rounded-[40px] p-12 shadow-2xl relative border border-white/20">
            ...
            <h2 className="text-3xl font-black text-slate-800 mb-2 tracking-tight">Nouvelle Salle</h2>
            <p className="text-slate-400 mb-8 text-sm font-medium">Enregistrez un nouvel espace d'enseignement</p>
            ...
            <form onSubmit={handleAddSalle} className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-2">Capacité</label>
                  <input 
              ...
              <div>
                ...
                <select 
                  className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none transition-all cursor-pointer font-bold text-slate-700"
              ...
              <div className="flex space-x-4 pt-6">
                ...
                <button type="submit" className="flex-1 py-5 bg-[#1d76f2] text-white font-black rounded-2xl shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all">Enregistrer</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {editModal.show && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[999] p-6">
          <div className="bg-white w-full max-w-xl rounded-[40px] p-12 shadow-2xl relative border border-white/20 border-blue-100">
            ...
            <h2 className="text-3xl font-black text-slate-800 mb-2 tracking-tight italic">Modifier la Salle</h2>
            <p className="text-slate-400 mb-8 text-sm font-medium">Mise à jour des informations de l'espace</p>
            ...
            <form onSubmit={handleUpdateSalle} className="space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-2">Département</label>
                <select 
                  className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer font-bold text-slate-700"
              ...
              <div className="flex space-x-4 pt-6">
                ...
                <button 
                    type="submit" 
                    className="flex-1 py-5 bg-emerald-600 text-white font-black rounded-2xl shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all"
                >
                    Mettre à jour
                </button>
              </div>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[1000] p-6 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-[40px] p-10 shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200 relative">
            ...
            <div className="text-center mb-10">
              <h3 className="text-2xl font-black text-slate-800 mb-3 tracking-tight">Supprimer la salle ?</h3>
            ...
            <div className="flex space-x-4">
              ...
              <button 
                ...
                className="flex-1 py-4 bg-red-600 text-white font-bold rounded-2xl shadow-xl shadow-red-100 hover:bg-red-700 transition-all active:scale-95"
              >
                Supprimer
              </button>
            </div>
